...
switch ($opcion) 
{
	...
	<div id='divImpresion' style='display:block' >
     ...
     echo $imprimir;
   	...
   	break;
   	case 3:
	/******************* Aqui estoy formando la solicitud  en una ventana emergente Eduardo ******************/
	
	/******************Estos son los datos que envio para la busqueda de la solicitud******************/
   	//	$idexpediente=$_POST['idexpediente'];
		//$idsolicitud=$_POST['idsolicitud'];
		...
		include_once("clsSolicitudesPorPaciente.php");
		//recuperando los valores generales de la solicitud
		/****************Aqui estoy cargando los datos del paciente y lo general de la solicitud ***********************/
		$consulta=$objdatos->DatosGeneralesSolicitud($idhistorialclinico,$lugar);
		//print_r($consulta);
                $row = mysql_fetch_array($consulta);
		//obteniedo los datos generales de la solicitud
		//valores de las consultas
		...
		/************Aqui Cargo los Examenes de esta solkicitud********************/
		$consultadetalle=$objdatos->DatosDetalleSolicitud($idsolicitud,$lugar);
