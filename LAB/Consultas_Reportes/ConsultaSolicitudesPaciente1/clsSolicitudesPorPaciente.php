...
function DatosGeneralesSolicitud($idhistorialclinico,$lugar)
{  
   $con = new ConexionBD;
   if($con->conectar()==true) 
   {
	$query = "SELECT DISTINCT mnt_empleados.IdEmpleado AS IdMedico,NombreEmpleado AS NombreMedico, 	 	NombreSubServicio AS Origen,DatosClinicos,
		...
		WHERE sec_solicitudestudios.IdServicio ='DCOLAB'  AND sec_solicitudestudios.IdEstablecimiento=$lugar AND sec_historial_clinico.IdHistorialClinico=$idhistorialclinico
		";
		$result = mysql_query($query);
     if (!$result)
       return false;
  ...
  //lista de examenes de la solicitud
  function DatosDetalleSolicitud($idsolicitud,$lugar)
  {
	$con = new ConexionBD;
   if($con->conectar()==true) 
   {
...
AND lab_areasxestablecimiento.Condicion='H' AND sec_solicitudestudios.IdSolicitudEstudio=$idsolicitud AND sec_solicitudestudios.IdEstablecimiento=$lugar
