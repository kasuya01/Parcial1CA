	...
	if($con->conectar()==true){
		 $query = /*"SELECT lab_examenes.IdExamen,NombreExamen FROM lab_examenes
		       ...
		       AND lab_examenesxestablecimiento.Condicion='H' ORDER BY NombreExamen ASC ";*/
                    ...
                    "SELECT lcee.id,lcee.nombre_examen,  t03.idestandar
                    FROM mnt_area_examen_establecimiento maees
                    INNER JOIN lab_conf_examen_estab lcee ON maees.id= lcee.idexamen
  				  JOIN ctl_examen_servicio_diagnostico t03 ON (t03.id = maees.id_examen_servicio_diagnostico)
   ...
   if($con->conectar()==true)
   {
         ...
         $query = "WITH tbl_servicio AS (
                ...
                SELECT DISTINCT t02.nombre AS nombre_establecimiento,
					   case when (select id_tipo_establecimiento from ctl_establecimiento where configurado=true) in (1,30,31,14)
							 then   TO_CHAR(t01.fecha_solicitud, 'DD/MM/YYYY HH24:MI AM')
							 else   TO_CHAR(t01.fecha_solicitud, 'DD/MM/YYYY')
