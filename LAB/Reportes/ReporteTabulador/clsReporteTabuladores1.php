...
class clsReporteExamenesporServicio
{
	...
	$con = new ConexionBD;
		...
		{ $query="SELECT lab_areas.IdArea,NombreArea FROM lab_areas 
			...
			WHERE lab_areasxestablecimiento.IdEstablecimiento=$lugar AND lab_areas.IdArea<>'JEF' 
			AND lab_areas.IdArea<>'REC' AND lab_areasxestablecimiento.Condicion='H' ORDER BY NombreArea";
...
function NumeroDeRegistros($lugar){
   ...
   if($con->conectar()==true){
     $query ="SELECT lab_areas.IdArea,NombreArea FROM lab_areas 
	      INNER JOIN lab_areasxestablecimiento ON lab_areas.IdArea=lab_areasxestablecimiento.IdArea
	      WHERE lab_areasxestablecimiento.IdEstablecimiento=$lugar AND lab_areas.IdArea<>'JEF' 
		  AND lab_areas.IdArea<>'REC' AND lab_areasxestablecimiento.Condicion='H' 
          ORDER BY NombreArea";
