...
switch ($opcion) 
{
	case 1: 
		...
		for ($i=0;$i<$NumProce;$l++){
			$cadProce=$cadProce."sum(if(mnt_subservicio.IdServicio='=$arrayidcod[$i],1,0)) AS $arrayProce[$i],";
		}	
	//print_r($arrayidcod );
		while ($rowexa=mysql_fetch_array($Conexam)){
			...
			WHERE  lab_examenes.IdEstandar='S15' AND ";
			...
			if((empty($_POST['procedencia'])) and (empty($_POST['area'])) and (empty($_POST['fechainicio'])) and (empty($_POST['fechafin'])))
			{
				$ban=1;
			}
		...
		$imprimir="<table width='90%' border='1' align='center' >
						<tr style='background:#BBBEC9'>
							<td class='CobaltFieldCaptionTD' width='20%'><strong>DIA</strong></td>";
					for ($k=0;$k<$NumCod;$k++){
				$imprimir.="<td class='CobaltFieldCaptionTD' width='12%'><strong>".$arrayidcod[$k]."</strong></td>
				</tr></table>";}
