...
$con = new ConexionBD;
	if($con->conectar()==true){// SELECT IdEstablecimiento,Nombre FROM mnt_establecimiento WHERE IdTipoEstablecimiento='$Idtipo' ORDER BY Nombre
		$sqlText= "SELECT IdEstablecimiento,Nombre FROM mnt_establecimiento WHERE IdTipoEstablecimiento='$Idtipo' AND IdEstablecimiento = '$lugar' ORDER BY Nombre";		
...
function DatosPaciente($nec){
	$con = new ConexionBD;
	if($con->conectar()==true){
		$query_Search= 	"SELECT e.IdNumeroExp, CONCAT_WS( ' ', d.PrimerApellido, d.SegundoApellido, ',', d.PrimerNombre, d.SegundoNombre, d.TercerNombre ) AS Nombre,
                                CASE sexo
                                WHEN '1' THEN 'Masculino'
                                WHEN '2' THEN 'Femenino' END AS sexoconv, (YEAR( CURRENT_DATE ) - YEAR( FechaNacimiento )) - ( RIGHT( CURRENT_DATE, 5 ) < RIGHT( FechaNacimiento, 5 )) AS Edad, ConocidoPor,sexo
                                FROM mnt_datospaciente d
                                INNER JOIN mnt_expediente AS e ON e.idpaciente = d.idpaciente 
                                WHERE e.idnumeroexp ='$nec'";
        // echo $query_Search;        
	$query = mysql_query($query_Search) or die('La consulta fallo&oacute;: ' . mysql_error());
