...
function ObtenerFechaResultado1($IdSolicitud,$IdExamen,$lugar){
  $con = new ConexionBD;
	 if($con->conectar()==true) 
	 {	$query="SELECT FechaHoraReg FROM lab_resultados 
  ...
  function DatosExamenes($idsolicitud,$idarea,$lugar,$sexo,$idedad)
	{
	    ...
	    if($con->conectar()==true){
			...
			$query = "SELECT sec_detallesolicitudestudios.IdDetalleSolicitud,sec_detallesolicitudestudios.IdExamen,lab_examenes.NombreExamen,
				  ...
				  WHERE sec_detallesolicitudestudios.IdSolicitudEstudio=$idsolicitud AND sec_historial_clinico.IdEstablecimiento=$lugar AND sec_detallesolicitudestudios.IdExamen LIKE'$idarea%'AND 		
				 lab_examenesxestablecimiento.IdPlantilla='A' AND sec_detallesolicitudestudios.EstadoDetalle='PM' AND lab_datosfijosresultado.IdEstablecimiento=$lugar
