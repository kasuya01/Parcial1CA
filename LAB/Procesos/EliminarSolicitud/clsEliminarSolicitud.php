...
class clsEliminarSolicitud
{
	 //constructor	
	 function clsEliminarSolicitud()
	 {
	 }	
...
function DatosDetalleSolicitud($idsolicitud)
{
  ...
  function consultarpag($query_search,$RegistrosAEmpezar,$RegistrosAMostrar)
 {
  ...
  function EliminarSolicitud($idresultado){
     $con = new ConexionBD;
    if($con->conectar()==true)
    {
...
/*function EliminarSolicitudMetodologia($iddetalle){
...
function ObtenerIdResultado($idsolicitud,$iddetalle){
 ...
 function EliminarSolicitudTarjeta($iddetalleres){
  $con = new ConexionBD;
	if($con->conectar()==true)
	{
