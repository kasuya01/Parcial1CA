...
switch ($opcion) 
{
  case 1: 
	...
	$query="SELECT sec_solicitudestudios.IdSolicitudEstudio,NumeroMuestra,sec_solicitudestudios.IdNumeroExp,lab_examenes.idexamen,
		nombreexamen,Indicacion,sec_detallesolicitudestudios.IdTipoMuestra,mnt_establecimiento.Nombre,
		mnt_establecimiento.Nombre,mnt_servicio.NombreServicio, mnt_subservicio.NombreSubServicio,
		...
		DATE_FORMAT(FechaRecepcion,'%d-%m-%Y')  as FechaRecepcion, sec_solicitudestudios.FechaHoraReg,
		lab_recepcionmuestra.FechaCita,
		...
		WHERE (estadodetalle='D' OR estadodetalle='PM') AND sec_detallesolicitudestudios.IdEstablecimiento=$lugar
	...
	AND (empty($_POST['SApellido'])) AND (empty($_POST['idexamen'])) AND (empty($_POST['idexamen'])) 
	AND (empty($_POST['TipoSolic'])))
		...
		if ($ban==0){
			...
			$query_search = $query. " ORDER BY lab_recepcionmuestra.NumeroMuestra DESC";
		//echo $query_search ;
...
$imprimir="<form name='frmDatos'>
	...
	//	$objdatos->insertar_encabezado($idsolicitud,$iddetalle,$idexamen,$idrecepcion,$responsable,$usuario,$tab,$lugar);
		...
		if ($objdatos->CambiarEstadoDetalle($idsolicitud,$estado,$idarea,$idtipo,$observacion,$idexamen)==true){
			if($objdatos->CambiarEstadoSolicitud($idexpediente,$idsolicitud,$estadosolicitud)==true){
