...
////	Procesos para el otorgamiento de CITAS PARA LOS SERVICIOS DE APOYO.	
	...
	if ($_POST['idservicio']== 'DCOLAB'){//Proceso para dar cita al area de laboratorio
	if (isset($_POST['fecha'])){
		...
		$Fecha='';
			...
			$dtSub=$citaserv->ListExamenesTiempoPrev($_POST['idsolicitudestudio'],$IdEstablecimiento);
                        $IdSubServicio=$citaserv->ObtenerSubServicio($_POST['idsolicitudestudio'],$Fecha);
			
			$Duplos=$citaserv->Duplos($_POST["idsolicitudestudio"],$IdSubServicio);
			

			if($row2=mysql_fetch_array($Duplos)){
				if($row2['Estado']=='C'){
				...
				$rslt.='<tr><td align="center" style="color:#990000; font:bold"><h3>'.mysql_num_rows($dtSub).'</h3></td></tr>
				...
				while ($row =mysql_fetch_array($dtSub)){
					$array_labexamens[$i] = $row[0];
					$array_namexams[$i] = $row[1];
					$i++;
				}
				...
				$dd_encontrado=array_search($codigo,$array_labexamens);
				...
				while ($row =mysql_fetch_array($dt_Cm)){
			...
			if (!empty($fecha)){
				...
				while($bandera==0){	
					...
					$creatinina = $citaserv->ContarCreatinina($actual);
