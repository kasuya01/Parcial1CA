...
switch ($opcion) 
{
  case 1: 
	...
	if ($Codigo==""){
		...
		lab_examenes.IdExamen,nombreexamen,Indicacion,DATE_FORMAT(lab_recepcionmuestra.FechaRecepcion,'%d/%m/%Y') AS FechaRecepcion,
		mnt_establecimiento.Nombre as Establecimiento,mnt_subservicio.NombreSubServicio,mnt_servicio.NombreServicio,
		CONCAT_WS(' ',PrimerNombre,NULL,SegundoNombre,NULL,PrimerApellido,NULL,SegundoApellido) AS Paciente,
		IF(sec_solicitudestudios.IdTipoSolicitud='S','URGENTE','NORMAL') AS prioridad,IdEstandar 
		...
		// $estadodetalle='D';  //estado en que la muestra ha sido tomada
			...
			if((empty($_POST['idexpediente'])) AND (empty($_POST['idarea'])) AND (empty($Nfecharecep)) AND (empty($_POST['IdEstab'])) 
			AND (empty($_POST['IdServ'])) AND (empty($_POST['IdSubServ'])) AND (empty($_POST['idexamen'])) AND (empty($_POST['PNombre'])) 
			AND (empty($_POST['SNombre'])) AND (empty($_POST['PApellido'])) AND (empty($_POST['SApellido'])) AND (empty($_POST['TipoSolic']))) 
			{ 
				$ban=1;
			}
		...
		{ 
			   echo"<tr>
						...
						"<input name='idexamen[".$pos."]' id='idexamen[".$pos."]' type='hidden' size='60' value='".$row['IdExamen']."' />".
                                                "<input name='IdEstandar[".$pos."]' id='IdEstandar[".$pos."]' type='hidden' size='60' value='".$row['IdEstandar']."' />".
		...
		if((empty($idsolicitud)) AND (empty($_POST['idarea'])) AND (empty($idestadar)) AND (empty($_POST['TipoSolic'])) AND (empty($idestab)))
			{
				$ban=1;
			}
   	...
   	case 2:
			...
			if($idarea=="URI" or $idarea=="BAT"){   
                                ...
                                <tr>
                                 ...
                                 <tr>
                                        ...
                                        <td class='StormyWeatherDataTD'>".htmlentities($origen)."
                                                ...
                                                <input name='fechaNac' id='fechaNac'  type='hidden' size='40' value='".$fechaNac."' disabled='disabled' />
                                        </td>
                                </tr>
                                <tr>
                                ...
                                <tr><td class='StormyWeatherFieldCaptionTD'>Peso:</td>";
                                   if(!empty($Peso))
