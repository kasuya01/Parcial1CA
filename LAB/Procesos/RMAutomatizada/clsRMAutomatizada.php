...
{  
   ...
   if($con->conectar()==true) 
   {
	 $query = "SELECT B.IdSolicitudEstudio AS IdSolicitudEstudio,NumeroMuestra,C.IdEmpleado AS IdMedico, 
                   NombreEmpleado AS NombreMedico, NombreSubServicio AS Origen, NombreServicio AS Precedencia, 
                   D.IdNumeroExp, CONCAT_WS(' ',PrimerNombre,NULL,SegundoNombre,NULL,PrimerApellido,NULL,SegundoApellido) AS NombrePaciente, 
                   CURDATE() AS Fecha,(year(CURRENT_DATE)-year(FechaNacimiento))AS Edad, IF(Sexo=1,'Masculino','Femenino') AS Sexo, 
	...
	function ExamenesPorArea($idarea,$lugar)
	 {
	    ...
	    if($con->conectar()==true){
	     $query = "SELECT lab_examenes.IdExamen,NombreExamen 
				   ...
				   WHERE IdArea='$idarea' AND lab_examenesxestablecimiento.IdEstablecimiento=$lugar
