...
function BuscarEstablecimientoExterno($IdEmpleado,$lugar){
     $con = new ConexionBD;
   if($con->conectar()==true){ 
       	$query="SELECT IdEstablecimientoExt 
            FROM mnt_empleados 
            WHERE IdEmpleado='$IdEmpleado' AND IdEstablecimiento=$lugar";
        //echo $query;
         $result = mysql_query($query);
           if (!$result)
       return false;
...
function ObtenerNombreCodigo($cod){
 ...
 //FUNCION PARA GUARDAR RESULTADOS
  //function InsertarResultadoPlantillaA($idexamen,$idsolicitud,$iddetalle,$resultado,$lectura,$observacion,$responsable,$lectura,$idrecepcion,$interpretacion,$observacion,$usuario,$codigo,$lugar,$IdEstabExt)
  function InsertarResultadoPlantillaA($idexamen,$idsolicitud,$iddetalle,$resultado,$responsable,$idrecepcion,$observacion,$usuario,$codigo,$lugar,$IdEstabExt)
 {
   $con = new ConexionBD;
   if($con->conectar()==true) 
  {	$query="INSERT INTO lab_resultados(IdSolicitudEstudio,IdDetalleSolicitud,IdExamen,
	...
	VALUES($idsolicitud,$iddetalle,'$idexamen',$idrecepcion,'$resultado','NULL',
		'NULL','$observacion','$responsable',$usuario,NOW(),$usuario,NOW(),$codigo,$IdEstabExt)";	
    // echo $query;
     $result = @mysql_query($query);
     if (!$result)
       return false;
