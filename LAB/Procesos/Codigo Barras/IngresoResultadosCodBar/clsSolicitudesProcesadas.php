...
$con = new ConexionBD;
 ...
 function MostrarDatosFijosPlantillaA($idexamen,$lugar)
 {
	 $con = new ConexionBD;
	   if($con->conectar()==true) 
	  {	$query="SELECT lab_examenes.IdExamen,NombreExamen,Unidades,RangoInicio,RangoFin 
			...
			WHERE IdEstablecimiento=$lugar AND lab_examenes.IdExamen='$idexamen' AND CURDATE() BETWEEN FechaIni AND IF(lab_datosfijosresultado.FechaFin ='0000-00-00',CURDATE(),lab_datosfijosresultado.FechaFin)";	
	     $result = @mysql_query($query);
	     if (!$result)
	       return false;
	     else
  ...
  function DatosExamenes($idsolicitud,$idarea,$lugar)
	{
		//echo $idsolicitud;
		$con = new ConexionBD;
	    ...
	    if($con->conectar()==true){
			...
			$query = "SELECT sec_detallesolicitudestudios.IdDetalleSolicitud,sec_detallesolicitudestudios.IdExamen,lab_examenes.NombreExamen,
				  lab_examenes.IdArea,sec_solicitudestudios.IdEstablecimiento, lab_datosfijosresultado.Unidades,lab_datosfijosresultado.RangoInicio, 
				  lab_datosfijosresultado.RangoFin,lab_examenes.IdArea,sec_solicitudestudios.IdEstablecimiento   
				  ...
				  WHERE sec_detallesolicitudestudios.IdSolicitudEstudio=$idsolicitud AND sec_detallesolicitudestudios.IdExamen LIKE'$idarea%'AND 		
				  lab_examenes.IdPlantilla='A' AND sec_detallesolicitudestudios.EstadoDetalle='PM' AND lab_datosfijosresultado.IdEstablecimiento=$lugar
				  AND CURDATE() BETWEEN FechaIni AND IF(FechaFin ='0000-00-00',CURDATE(),FechaFin) ORDER BY NombreExamen";
			//echo $query;
			 $result = @mysql_query($query);
		 if (!$result)
		   return false;
...
function ObtenerNombreCodigo($cod){
 ...
 function BuscarEmpleados($idarea,$lugar)
 {
  ...
  {	$query="SELECT IdEmpleado,NombreEmpleado 
		...
		IdArea='$idarea' AND (mnt_cargoempleados.IdCargoEmpleado=2 OR 
		mnt_cargoempleados.IdCargoEmpleado=1) AND IdEstablecimiento=$lugar";	
     	$result = mysql_query($query);
     	if (!$result)
       		return false;
     	else
 ...
 function MostrarResultadoGenerales($idsolicitud,$idexamen,$lugar)
 {
  ...
  {	$query="SELECT lab_recepcionmuestra.IdSolicitudEstudio, mnt_expediente.IdNumeroExp, CONCAT_WS(' ',PrimerNombre,NULL,SegundoNombre,NULL,PrimerApellido,NULL,SegundoApellido) AS NombrePaciente,
	...
	TelefonoCasa,Direccion,NombreSubServicio AS Origen,NombreServicio AS Procedencia,NombreArea,NumeroMuestra,DATE_FORMAT(CURRENT_DATE,'%d/%m/%Y') AS Fecha,mnt_establecimiento.Nombre,DATE_FORMAT(FechaNacimiento,'%d/%m/%Y') as FechaNacimiento
	FROM sec_detallesolicitudestudios 
 ...
 function MostrarResultadoGenerales1($idsolicitud,$idarea,$lugar)
 {
  ...
  {	$query="SELECT lab_recepcionmuestra.IdSolicitudEstudio, mnt_expediente.IdNumeroExp, 
		...
		NombreArea,NumeroMuestra,DATE_FORMAT(CURRENT_DATE,'%d/%m/%Y') AS Fecha,sec_solicitudestudios.IdEstablecimiento, DATE_FORMAT(FechaNacimiento,'%d/%m/%Y') AS FechaNacimiento
		FROM sec_detallesolicitudestudios 
		INNER JOIN sec_solicitudestudios ON sec_solicitudestudios.IdSolicitudEstudio=sec_detallesolicitudestudios.IdSolicitudEstudio
		INNER JOIN lab_recepcionmuestra ON lab_recepcionmuestra.IdSolicitudEstudio=sec_solicitudestudios.IdSolicitudEstudio
		INNER JOIN sec_historial_clinico ON sec_historial_clinico.IdHistorialClinico=sec_solicitudestudios.IdHistorialClinico
 ...
 function MostrarResultadoPlantillaA($idsolicitud,$idexamen)
 {
   $con = new ConexionBD;
   if($con->conectar()==true) 
  {	$query="SELECT C.IdSolicitudEstudio, H.IdNumeroExp,CONCAT_WS(' ',PrimerNombre,NULL,SegundoNombre,NULL,PrimerApellido,NULL,SegundoApellido) AS NombrePaciente,
			...
			Unidades,A.Observacion,Responsable,NombreEmpleado,current_date as Fecha
...
function DetalleExamenes($idsolicitud,$idarea,$lugar)
 {
		//echo $idsolicitud;
		$con = new ConexionBD;
	    //usamos el metodo conectar para realizar la conexion
	    if($con->conectar()==true){
			...
			WHERE sec_detallesolicitudestudios.IdSolicitudEstudio=$idsolicitud AND lab_examenes.IdExamen LIKE'$idarea%'AND lab_examenes.IdPlantilla='A' 
