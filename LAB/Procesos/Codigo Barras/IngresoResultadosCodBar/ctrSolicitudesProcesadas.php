	...
	case 1:  
		...
		if ($Codigo==""){
		 		$query = "SELECT IdPlantilla,sec_detallesolicitudestudios.IdDetalleSolicitud, 		
		            	  ...
		            	  NombreSubServicio,NombreServicio, sec_solicitudestudios.Impresiones,mnt_establecimiento.Nombre,sec_historial_clinico.IdHistorialClinico,sec_solicitudestudios.FechaSolicitud  
					      ...
					      WHERE  estadodetalle='PM' AND  lab_recepcionmuestra.FechaRecepcion=CURRENT_DATE AND  sec_solicitudestudios.IdEstablecimiento=$lugar AND ";
					$ban=0;	
				...
				if((empty($_POST['idexpediente'])) AND (empty($_POST['idarea'])) AND (empty($_POST['fechasolicitud'])) AND (empty($_POST['IdEstab'])) AND (empty($_POST['IdServ'])) AND (empty($_POST['IdSubServ'])) AND (empty($_POST['PNombre'])) AND (empty($_POST['SNombre'])) AND (empty($_POST['PApellido'])) AND (empty($_POST['SApellido'])) AND (empty($_POST['idexamen'])))
				{
						$ban=1;
				}
