...
function CargarDatosFormulario(idexpediente,idsolicitud,idarea,establecimiento )
{
	...
	ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
		  //enviando los valores
	ajax.send("opcion="+opcion+"&idarea="+idarea+"&idexpediente="+idexpediente+"&idsolicitud="+idsolicitud+"&idexamen+"+idexamen+"&establecimiento="+establecimiento);
	ajax.onreadystatechange=function() 
	{
		if (ajax.readyState==4) 
		{	if (ajax.status == 200)
...
function MuestrasPendientes()
{
	...
	ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
		  //enviando los valores
	ajax.send("opcion="+opcion+"&idarea="+idarea+"&idexpediente="+idexpediente+"&fecharecep="+fecharecep+"&IdEstab="+IdEstab+"&IdServ="
	+IdServ+"&IdSubServ="+IdSubServ+"&idexamen="+idexamen+"&PNombre="+PNombre+"&SNombre="+SNombre+"&PApellido="+PApellido+"&SApellido="
	+SApellido+"&Codigo="+Codigo+"&Orden="+Orden);	
	ajax.onreadystatechange=function() 
	{
		if (ajax.readyState==4) 
		{	if (ajax.status == 200)
