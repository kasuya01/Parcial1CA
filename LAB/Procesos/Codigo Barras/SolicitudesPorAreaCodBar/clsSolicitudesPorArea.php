...
{  
   ...
   if($con->conectar()==true) 
   {
	...
	sec_solicitudestudios.FechaSolicitud,mnt_establecimiento.Nombre,DATE_FORMAT(FechaNacimiento,'%d/%m/%Y') as FechaNacimiento FROM sec_historial_clinico 
  ...
  function DatosDetalleSolicitud($idarea,$idsolicitud,$idtipo)
...
function CambiarEstadoDetalle($idsolicitud,$estado,$idarea,$idtipo)
 {
   $con = new ConexionBD;
   if($con->conectar()==true) 
   {
...
WHERE IdArea='$idarea'
AND lab_examenesxestablecimiento.Condicion='H'  AND lab_examenesxestablecimiento.IdEstablecimiento=$lugar ORDER BY NombreExamen ASC";
		 $result = @mysql_query($query);
		 if (!$result)
		   return false;
		 else
