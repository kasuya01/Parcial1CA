...
function MostrarDatosGenerales($idsolicitud)
{
   ...
   if($con->conectar()==true)
   {
     $query = "SELECT DISTINCT lab_recepcionmuestra.IdSolicitudEstudio, sec_historial_clinico.IdNumeroExp,
				...
				NumeroMuestra,
				DATE_FORMAT(CURRENT_DATE,'%d/%m/%Y') AS Fecha,DATE_FORMAT(mnt_datospaciente.FechaNacimiento,'%d/%m/%Y') as FechaNacimiento
...
function LeerProcesoExamen($idexamen)
{
	$con = new ConexionBD;
   if($con->conectar()==true) 
   {
	$query ="SELECT idprocedimientoporexamen,nombreprocedimiento,unidades,rangoinicio,rangofin FROM lab_procedimientosporexamen
