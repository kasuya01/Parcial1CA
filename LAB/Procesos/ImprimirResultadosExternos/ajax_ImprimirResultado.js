...
function MostrarDatos(posicion)
{
		...
		CargarDatosFormulario(idexpediente,idsolicitud,IdEstablecimiento,idarea);
	 
 }

 function CargarDatosFormulario(idexpediente,idsolicitud,IdEstablecimiento,idarea)
{
	ajax=objetoAjax();
	opcion=3;
...
ajax.send("opcion="+opcion+"&idexpediente="+idexpediente+"&idsolicitud="+idsolicitud+"&IdEstablecimiento="+IdEstablecimiento+"&idarea="+idarea);
	ajax.onreadystatechange=function() 
	{
		if (ajax.readyState==4) 
		{	 if (ajax.status == 200)
