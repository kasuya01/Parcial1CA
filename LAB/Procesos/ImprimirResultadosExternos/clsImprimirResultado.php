...
function BuscarEstablecimientoExterno($IdEmpleado,$lugar){
   ...
   if($con->conectar()==true){ 
       	$query="SELECT IdEstablecimientoExt 
            FROM mnt_empleados 
            WHERE Correlativo='$IdEmpleado' AND IdEstablecimiento=$lugar";
      // echo $query;
         $result = mysql_query($query);
           if (!$result)
       return false;
...
function DatosDetalleSolicitud($idexpediente,$idsolicitud,$idarea)
{
   ...
   if($con->conectar()==true) 
   {
	   $query = "SELECT sec_solicitudestudios.IdNumeroExp,  lab_examenes.IdArea AS IdArea,
		...
		WHERE sec_solicitudestudios.IdServicio ='DCOLAB' AND lab_examenes.IdArea='$idarea' AND lab_areasxestablecimiento.Condicion='H' AND sec_solicitudestudios.IdNumeroExp='$idexpediente' AND sec_solicitudestudios.IdSolicitudEstudio=$idsolicitud 
