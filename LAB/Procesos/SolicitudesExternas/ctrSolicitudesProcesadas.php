...
$conExt=$objdatos->BuscarEstablecimientoExterno($Empleado,$lugar);
$EstablecimientoExt= mysql_fetch_array($conExt);
//echo $EstablecimientoExt[0];
//variables POST
$opcion=$_POST['opcion'];
...
switch ($opcion) 
{
  case 1:  
	...
	$TipoSolic=$_POST['TipoSolic'];
        //echo $idarea;
        $ban=0;
        $query = "SELECT lab_examenesxestablecimiento.IdPlantilla,sec_detallesolicitudestudios.IdDetalleSolicitud, sec_solicitudestudios.IdSolicitudEstudio,NumeroMuestra, 	
	   	  sec_solicitudestudios.IdNumeroExp,lab_recepcionmuestra.IdRecepcionMuestra,
		  lab_recepcionmuestra.FechaRecepcion, lab_examenes.idexamen, lab_examenes.NombreExamen,
		  ...
		  mnt_establecimiento.Nombre,IF(sec_solicitudestudios.IdTipoSolicitud='S','URGENTE','NORMAL') AS prioridad,
                  FechaNacimiento,Sexo,IdEstandar,sec_historial_clinico.IdHistorialClinico   
		  ...
		  WHERE (sec_detallesolicitudestudios.EstadoDetalle='D' OR sec_detallesolicitudestudios.EstadoDetalle='PM') 
 ...
 $pos=0;
       ...
       echo "<tr>
		...
		"</td>". 
			...
			"<input name='establecimiento[".$pos."]' id='establecimiento[".$pos."]' type='hidden' size='60' value='".htmlentities($row["Nombre"])."' />".
                        ...
                        "<input name='IdEstabExt[".$pos."]' id='IdEstabExt[".$pos."]' type='hidden' size='60' value='".$EstablecimientoExt[0]."'/>".
   ...
   case 3://GUARDANDO DATOS DE RESULTADOS Y MOSTANDO LISTA ACTUALIZADA PLANTILLA "A"
	...
	$fecharecep=$_POST['fecharecep'];
	$codigo=6;
        $IdEstabExt=$_POST['IdEstabExt'];
        //echo $codigo;
   //Guardando Resultados
  // if($objdatos->InsertarResultadoPlantillaA($idexamen,$idsolicitud,$iddetalle,$resultado,$lectura,$observacion,$responsable,$lectura,$idrecepcion,$interpretacion,$observacion,$usuario,$codigo,$lugar,$IdEstabExt)==true)      
    if($objdatos->InsertarResultadoPlantillaA($idexamen,$idsolicitud,$iddetalle,$resultado,$responsable,$idrecepcion,$observacion,$usuario,$codigo,$lugar,$IdEstabExt)==true)
	...
	case 4: //MOSTRAR PREVIAMENTE LOS RESULTADOS 
		...
		$Imprimir="<table width='100%' align='center' class='StormyWeatherFormTABLE'>
			...
			$Imprimir.="
					...
					$Imprimir.="	<td align='justify' >".htmlentities($observacion)."</td>
						</tr>"; 
			$Imprimir.="					
			<tr>
				<td>&nbsp;</td>
			...
			<tr>
				...
				<td colspan='7' align='center' >
					...
					<input type='button' name='Imprimir'  id='Imprimir' value='Imprimir' Onclick='ImprimirPlantillaA(".$idsolicitud.",\"".$idexamen."\",\"".$iddetalle."\",\"".$resultado."\", \"".htmlentities($observacion)."\",\"".$responsable."\",\"".$sexo."\",\"".$idedad."\",\"".$IdEstabExt."\") ;'>
					<input type='submit' id='btnSalir' value='Cerrar' Onclick='Cerrar() ;' />
				</td>
			</tr></table>		
		";
                        //<input type='button' name='Imprimir'  id='Imprimir' value='Imprimir' Onclick='ImprimirPlantillaA(".$idsolicitud.",\"".$idexamen."\",\"".$iddetalle."\",\"".$resultado."\", \"".htmlentities($lectura)."\",\"".htmlentities($interpretacion)."\",\"".htmlentities($observacion)."\",\"".$responsable."\",\"".$sexo."\",\"".$idedad."\",\"".$IdEstabExt."\") ;'>
		echo $Imprimir;
