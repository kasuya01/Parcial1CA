...
$(document).ready(function () {
    ...
    $("select[name^='exarealizar_']").each(function() {
        alert ('llego aki');
...
$pil = $objdatos->Piloto($idexpediente,  $fechacita, $lugar, $idEstablecimiento, $idsollicitud ,$idestablecimientoext);
...
//echo "despues del for ".$idestablecimientoext;
    ...
    while ($row = pg_fetch_array($ConsultaDatos)) {
        $id_usuariosol=$row['id_usuariosol'];
        $username = $row['username'];
        echo '<input type="hidden" id="user_petition" name="user_petition" value="'.$username.'"';
        ...
        echo "<div class='box-body'>
                  ...
                  echo "<div class='table-responsive' style='width: 100%;' style='align:center;'>
                     <table border = 0 align='center' class='table table-hover table-bordered table-condensed table-white' cellspacing='0' style='width:100%'><thead>
          		        <tr>
                              <td colspan='10' align='center' class='CobaltFieldCaptionTD'>ESTUDIOS SOLICITADOS <a href='http://siaps.localhost/app_dev.php/laboratorio/solicitudexterna/send?id=12' target='_blank'>soap</a></td>
          		        </tr>
          		        <tr class='prueba'>
                              <th>Código Prueba</th>
                              <th>Cód. Area</th>
                              <th> Examen Solicitado </th>";
                   if ($username=='uwebservice' ){
                       echo "<th style='width:300px' id='colreferido_'> Examen A Realizar </th>";
                   }
                  
                  echo "         <th> Indicaciones </th>
                              <th >Fecha Toma Mx.<br/><input type='text' placeholder='aaaa-mm-dd HH:MM' class='datepicker form-control height' title='Seleccione la Fecha de toma de muestra igual para actualizar la de todas las pruebas.' id='fgentomamxgen'  name='fgentomamx' style='width:125px;  font-size:10px;!important' value='" . $fecha . "' onchange= \"valfechasolicita(this.value, 'fgentomamxgen'), updatealldates(), changefechatomamx()\"  ></th>
                              <th>A Realizar</th>                              
...
//echo "solicitud ".$row['idsolicitudestudio'];
                  ...
                  while ($rows = pg_fetch_array($detalle)) {
                    echo "<tr id='rowdetalle_".$k."'>
                              ...
                              <td>" . $rows['idarea'] . " </td>";
                    if ($username=='uwebservice' ){
                      //   echo "<td style='width:300px'>" . htmlentities($rows['nombre_examen_solicitado']) . "</td>";
                        echo "<td style='width:150px'>". htmlentities($rows['nombre_examen_solicitado']) . "</td>"; 
                        $posiblesexamenes = $objdatos->BuscarPosibleExamenes($rows['id_area_examen_estab']);
                        $cantposexa = pg_num_rows($posiblesexamenes);
                                if ($cantposexa>=1){
                                    echo '<td style="width:150px;">'
                                    . '<select id="exarealizar_'.$k.'" name="exarealizar_"  class="form-control height" style="width:150px; font-size:12px;!important" onchange="asignarIdConf(this.value, '.$k.')">';
                                    if ($cantposexa>1)
                                    echo '<option value="0" style="font-size:12px;!important" selected >Seleccione una opción..</option>';
                                    
                                }
                                else{
                                    if ($cantposexa==0){
                                        echo '<td style="width:150px" class="bg-danger">';
                                        echo '<input type="hidden" id="exarealizar_'.$k.'" name="exarealizar_" value="X"/> ';
                                        echo 'No puede continuar, No hay examenes configurados asociados a los solicitados.';
                                    }
                                }
                                while ($exar = @pg_fetch_array($posiblesexamenes)){
                                    echo '<option value = "'.$exar["id"].'" style = " font-size:12px;!important">'.$exar["nombre_examen"].'</option>';
                                }
                        echo '</td>';
                    }
                    else{
                     echo "<td style='width:300px'>" . htmlentities($rows['nombreexamen']) . "</td>";
                    }
                      ...
                      echo " <td style='width:125px;  font-size:12px;!important'><input type='text' class='datepicker form-control height'  id='f_tomamuestra_".$k."' name='f_tomamuestra_'  value='". date('Y-m-d H:i')."' onchange=\"valfechasolicita(this.value, 'f_tomamuestra_".$k."'); changenewdate('".$k."')\" style='width:125px;  font-size:12px;!important' />"
                      ...
                      if ($cantformas>=1){
                          echo '<td style="width:150px"><select id="arealizar_'.$k.'" name="arealizar_"  class="form-control height" style="width:150px;  font-size:12px;!important">';
                          if ($cantformas>1)
                      ...
                      echo '<select id="validarmuestra_'.$k.'" name="validarmuestra_" onchange="OpcionRechazo(this.value, '.$k.')" class="form-control height" style="width:200px;  font-size:12px;!important">';
