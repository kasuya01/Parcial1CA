 ...
 function LlenarResultados($IdEstandar)
 {
	 $con = new ConexionBD;
	  ...
	  {	$query="SELECT lab_codigosxexamen.IdResultado,Resultado
                        ...
                        WHERE lab_codigosxexamen.IdEstandar = '$IdEstandar'"	;	
	     $result = mysql_query($query);
	     if (!$result)
	       return false;
	     else
...
function MostrarDatosGenerales($idsolicitud,$lugar)
{
   ...
   if($con->conectar()==true)
   {
      $query = "SELECT DISTINCT lab_recepcionmuestra.IdSolicitudEstudio, sec_historial_clinico.IdNumeroExp,
				...
				NumeroMuestra,
				DATE_FORMAT(lab_recepcionmuestra.FechaHoraReg,'%d/%m/%Y %H:%i:%s') AS Fecha,
				DATE_FORMAT(mnt_datospaciente.FechaNacimiento,'%d/%m/%Y') as FechaNacimiento
   ...
   if($con->conectar()==true) 
   {
     $query="SELECT idprocedimientoporexamen,nombreprocedimiento,unidades,rangoinicio,rangofin 
			 ...
			 AND CURDATE() BETWEEN FechaIni AND IF(FechaFin ='0000-00-00',CURDATE(),FechaFin)
                         AND (lab_procedimientosporexamen.idsexo=$sexo OR lab_procedimientosporexamen.idsexo=3) 
