...
function MuestrasRechazadas()
{
	...
	ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
		  //enviando los valores
	ajax.send("opcion="+opcion+"&estado="+estado+"&fechasolicitud="+fechasolicitud+"&idarea="+idarea+"&idexpediente="+idexpediente+"&idsolicitud="+idsolicitud+"&idexamen="+idexamen+"&fecharecep="+fecharecep+"&IdEstab="+IdEstab+"&IdServ="+IdServ+"&IdSubServ="+IdSubServ+"&PNombre="+PNombre+"&SNombre="+SNombre+"&PApellido="+PApellido+"&SApellido="+SApellido);
	ajax.onreadystatechange=function() 
	{
		if (ajax.readyState==4) 
		{	 if (ajax.status == 200)
