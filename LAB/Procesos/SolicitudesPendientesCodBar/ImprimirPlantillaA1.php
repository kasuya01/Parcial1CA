...
include_once("clsSolicitudesProcesadas.php");
 ...
 // $consulta=$objdatos->DetalleExamenes($idsolicitud,$idarea,$lugar);
  $consulta=$objdatos->ImprimirResultadoPlantillaA1($idarea,$lugar,$idsolicitud);
...
?>
	...
	<table width='90%'  align='center' border='0' class='StormyWeatherFormTABLE'>
		...
		?>  <td align='center'> <?php echo htmlentities($rowdet['Resultado'])?></td>
			<td align='center'><?php echo htmlentities($rowdet['Unidades'])?></td>
			<td width='15%' align='left'><?php echo $rowdet['RangoInicio']."-".$rowdet['RangoFin']?></td>
			<!--<td><td><?php //echo htmlentities($vector_lecturas[$pos])?></td>-->
		      <!--<td>  <td><?php //echo htmlentities($vector_inter[$pos])?></td>-->
		    <td><?php echo htmlentities($rowdet['Observacion'])?></td>
			<td ><?php echo htmlentities($rowdet['FechaResultado'])?></td>
		</tr>
		<?php
		      //}
			$pos=$pos + 1;
