...
function ObtenerFechaResultado($idsolicitud,$idarea,$lugar)
{
 ...
 function CambiarEstadoSolicitud($idsolicitud){
  $con = new ConexionBD;
   if($con->conectar()==true){ 
	$query="SELECT IdDetalleSolicitud,IdExamen 
   ...
   if($con->conectar()==true) 
   {
    $query = "INSERT INTO lab_resultados
	         (IdSolicitudEstudio,IdDetalleSolicitud,IdExamen,IdRecepcionMuestra,     
			  ...
			  VALUES($idsolicitud,$iddetalle,'$idexamen',$idrecepcion,'$observacion','$responsable',
...
function MostrarDatosGenerales($idsolicitud,$lugar)
{
   ...
   if($con->conectar()==true)
   {
     $query = "SELECT DISTINCT  lab_recepcionmuestra.IdSolicitudEstudio, sec_historial_clinico.IdNumeroExp,
	   ...
	   NumeroMuestra,DATE_FORMAT(lab_recepcionmuestra.FechaHoraReg,'%d/%m/%Y %H:%i:%s') AS Fecha,mnt_establecimiento.Nombre,DATE_FORMAT(FechaNacimiento,'%d/%m/%Y') as FechaNacimiento
	   ...
	   WHERE  lab_recepcionmuestra.IdSolicitudEstudio=$idsolicitud and sec_solicitudestudios.IdEstablecimiento=$lugar AND mnt_expediente.IdEstablecimiento=$lugar";
     $result = @mysql_query($query);
     if (!$result)
       return false;
     else
...
//FUNCION PARA OBTENER ELEMENTOS DE UN EXAMEN PLANTILLA B
 ...
 function LeerSubElementosExamen($idelemento,$lugar)
 {
   $con = new ConexionBD;
   if($con->conectar()==true)
   {
     $query = "SELECT IdSubElemento,SubElemento,Unidad,ObservSubElem 
	 FROM lab_subelementos 
	 WHERE idelemento=$idelemento AND lab_subelementos.IdEstablecimiento=$lugar AND 
	 CURDATE() BETWEEN FechaIni AND IF(lab_subelementos.FechaFin ='0000-00-00',CURDATE(),lab_subelementos.FechaFin) 
