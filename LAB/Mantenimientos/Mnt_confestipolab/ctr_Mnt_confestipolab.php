...
function getconfestipolab($parameters) {
	...
	$arr = $mntconfestipolab->getconfestipolab($parameters['idarea'], $parameters['lugar']);
	...
	$array = array();
 	$b=0;
        if ($parameters['post']['form']!==''){
		   $setfechafin = $mntconfestipolab->setfechafin();
           ...
           foreach ($parameters['post']['form'] as $value) {
			...
			//    $array['idestipolab'][] = $tmp_array[1];
             ...
             /*  if($tmp_array[1] !== 'na') {
			   ...
			   $query = $mntconfestipolab->setconfestipolab($parameters['lugar'], $tmp_array, $parameters['usuario']);
