...
class clsLab_TarjetasVITEK
{
 //constructor	
	...
	function consultarpag($RegistrosAEmpezar, $RegistrosAMostrar,$lugar){
		...
		if($con->conectar()==true){
			$query = "SELECT id,nombretarjeta, TO_CHAR(fechaini::timestamp, 'DD/MM/YYYY') AS fechaini, TO_CHAR(fechafin::timestamp, 'DD/MM/YYYY') AS fechafin FROM  lab_tarjetasvitek
	...
	function consultarid($idtarjeta,$lugar)
	{
		...
		if($con->conectar()==true)
		{
			$query = "SELECT id,nombretarjeta, TO_CHAR(fechaini::timestamp, 'DD/MM/YYYY') AS fechaini, TO_CHAR(fechafin::timestamp, 'DD/MM/YYYY')  FROM lab_tarjetasvitek WHERE id='$idtarjeta' AND idestablecimiento=$lugar";
			$result = pg_query($query);
			if (!$result)
				return false;
			else
