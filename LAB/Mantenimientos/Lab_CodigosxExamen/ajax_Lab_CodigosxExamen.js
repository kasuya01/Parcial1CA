...
function BuscarCodigosEstandarxGrupo(idgrupo)
{
	var opcion=5;
       //instanciamos el objetoAjax
	ajax=objetoAjax();
	//archivo que realizar� la operacion ->actualizacion.php
     	ajax.open("POST", "ctrLab_CodigosxExamen.php",true);
	//muy importante este encabezado ya que hacemos uso de un formulario
	ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
	//enviando los valores
        

          alert (idgrupo+"--"+opcion);
	ajax.send("idgrupo="+idgrupo+"&opcion="+opcion);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
			//mostrar los nuevos registros en esta capa
			document.getElementById('cmbEstandar').innerHTML = ajax.responseText;
		}
	}	
}

function LimpiarCampos(){
	document.getElementById('cmbgrupo').value="0";
	document.getElementById('cmbEstandar').value="0";
...
function IngresarRegistro(){ //INGRESAR REGISTROS
	...
	idresultado=document.getElementById('cmbResultado').value;
       
	alert (idestandar+"--"+idresultado);
	var opcion=1;
	Pag=1;
	//instanciamos el objetoAjax
	ajax=objetoAjax();
	...
	ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
	//enviando los valores
	ajax.send("idestandar="+idestandar+"&idresultado="+idresultado+"&idgrupo="+idgrupo+"&opcion="+opcion);
}
...
function pedirDatos(iddatosfijosexamen){ //CARGAR DATOS A MODIFICAR
	...
	ajax.open("POST", "consulta_CodigosxExamen.php");
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
			//mostrar resultados en esta capa
...
function eliminarDato(iddatosfijosresultado){ //FUNCION PARA ELIMINACION
	...
	if ( eliminar ) {
		...
		ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
		//enviando los valores
		ajax.send("opcion="+opcion+"&IdCodigosxExameno="+IdCodigosxExamen+"&Pag="+Pag);
		ajax.onreadystatechange=function() {
			if (ajax.readyState==4) {
				//mostrar resultados en esta capa
				//divResultado.innerHTML = ajax.responseText;
...
function BuscarDatos()
{
	...
	ajax.send("&idestandar="+idestandar+
                "&idresultado="+idresultado+"&Pag="+Pag);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
			//mostrar los nuevos registros en esta capa
