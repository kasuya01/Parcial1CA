<?php session_start();
include_once("clsLab_CodigosxExamen.php");
...
switch ($opcion) 
{
	case 1:  //INSERTAR	
		...
		$IdResultado=$_POST['idresultado'];
               echo $IdEstandar."--".$IdResultado."--".$Idgrupo;
		if ($objdatos->insertar($IdEstandar,$IdResultado)==true) 
	...
	break;
    	case 2:  //MODIFICAR      
			$IdCodigosxExamen=$_POST['IdCodigosxExamen'];
			$IdEstandar=$_POST['idestandar'];
                        $IdResultado=$_POST['idresultado'];
			
								
			if ($objdatos->actualizar($IdCodigosxExamen,$IdEstandar,$IdResultado)==true) 
		...
		//muestra los datos consultados en la tabla
		  echo "<table border = 1 align='center' class='StormyWeatherFormTABLE'>
			<tr>
				...
				<td class='CobaltFieldCaptionTD'> Código Prueba </td>
				<td class='CobaltFieldCaptionTD'> Nombre Examen </td>
				<td class='CobaltFieldCaptionTD'> Código de Resultado </td>	   
				<td class='CobaltFieldCaptionTD'> Resultado </td>
		...
		while($row = mysql_fetch_array($consulta)){
		  echo "<tr>
				<td aling='center'> 
                                    ...
                                    onclick=\"pedirDatos('".$row[4]."')\"> </td>
				<td aling ='center'> 
                                    <img src='../../../Iconos/eliminar.gif' style=\"text-decoration:underline;cursor:pointer;\" 
                                    onclick=\"eliminarDato('".$row[4]."')\"> </td>
				<td> $row[0]</td>
				<td>".htmlentities($row[1])."-".$row[4]."</td>
	...
	case 5:  
		$idgrupo=$_POST['idgrupo'];
           	...
           	echo $idgrupo; 
	  	$rslts='';
		$conEstandar = $objdatos->EstandaresxGrupo($idgrupo);
		//$dtMed=$obj->LlenarSubServ($proce);	
		
		$rslts = '<select name="cmbEstandar" id="cmbEstandar" size="1" >';
		$rslts .='<option value="0">--Seleccione un Código--</option>';
			
		while ($rows =mysql_fetch_array($conEstandar)){
			$rslts.= '<option value="'.$rows[0].'" >'.$rows[0].' - '.htmlentities($rows[1]).'</option>';
		}
