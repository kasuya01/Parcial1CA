...
class clsLab_Areas
{
 ...
 function insertar($idarea,$nombrearea,$usuario,$lugar)
 {
   $con = new ConexionBD;
   if($con->conectar()==true) 
   {
    $query = "INSERT INTO lab_areas(IdArea,NombreArea,IdUsuarioReg,FechaHoraReg,IdUsuarioMod,FechaHoraMod) VALUES('$idarea','$nombrearea','$usuario',NOW(),'$usuario',NOW())";
...
function actualizar($idarea,$nom,$usuario)
 {
   $con = new ConexionBD;
   if($con->conectar()==true) 
   {
    
     $query = "UPDATE lab_areas SET NombreArea='$nom',IdUsuarioMod='$usuario',FechaHoraMod=NOW() WHERE IdArea='$idarea'";	
     $result = @mysql_query($query);
...
INNER JOIN lab_areasxestablecimiento ON lab_areas.IdArea= lab_areasxestablecimiento.IdArea
WHERE lab_areasxestablecimiento.Condicion='H' AND lab_areasxestablecimiento.IdEstablecimiento=$lugar AND Administrativa='N' ORDER BY NombreArea";
	 $result = @mysql_query($query);
	 if (!$result)
	   return false;
	 else
