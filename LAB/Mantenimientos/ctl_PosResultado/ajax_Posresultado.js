...
function LimpiarCampos(){
	document.frmnuevo.txtposibleresultado.value="";
	//document.frmnuevo.txtantibiotico.value="";
	document.frmnuevo.txtposibleresultado.focus();
}

function IngresarRegistropos(){ //INGRESAR REGISTROS	
	/*if (DatosCompletos())
	{*/
		...
		posreusltado=document.frmnuevo.txtposibleresultado.value;
		var opcion=1;
		var Pag=1;
		...
		//archivo que realizar� la operacion
                //alert(posreusltado);
		ajax.open("POST", "ctrLab_Posresultados.php",true);
		...
		ajax.send("&posreusltado="+posreusltado+"&Pag="+Pag+"&opcion="+opcion);		
	/*}
	else{
...
function pedirDatos(idposres){   //CARGAR DATOS A MODIFICAR
	...
	ajax=objetoAjax();
	//uso del medotod POST
	ajax.open("POST", "consulta_Posresultado.php");
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
			//mostrar resultados en esta capa
	...
	ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
	//enviando el codigo 
	ajax.send("idposres="+ idposres );
	
}

function enviarDatosposresultado(){    //FUNCION PARA MODIFICAR
	//donde se mostrar� lo resultados
	divResultado = document.getElementById('divinicial');
	divFormulario = document.getElementById('divFrmModificar');
	...
	posresultado=document.frmModificar.posresultado.value;
        idposresultado=document.frmModificar.idposresultado.value;
	...
	ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
	//enviando los valores
	ajax.send("&idposresultado="+idposresultado+"&posresultado="+posresultado+"&cmbEstado="+cmbEstado+"&Pag="+Pag+"&opcion="+opcion);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
...
function BuscarPosresultados()
{
	var opcion=5;
	Pag=1;
        //valores de los cajas de texto
	//idantibiotico=document.getElementById('txtidantibiotico').value;
	txtposibleresultado=document.getElementById('txtposibleresultado').value;
	//instanciamos el objetoAjax
	ajax=objetoAjax();
	...
	ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
	//enviando los valores
	ajax.send("&txtposibleresultado="+txtposibleresultado+"&Pag="+Pag+"&opcion="+opcion);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
			//mostrar los nuevos registros en esta capa
